module top(
  ...
  always_ff @(posedge clk) begin
    if (btnC) begin
      if (~out_empty) begin
        ...
        // ram[17] <= wdata_proc;
        is_write <= 1'b1;
    end else if (pgm_mem_pedge && ~out_empty) begin
      ram[mem_sel] <= out_data;
    end
  ...
  always_latch begin
    ...
    if (is_btnD_ || is_btnU_ || is_load || is_btnC_ || ~is_CPU_RESETN || pgm_mem_pedge) begin
        mem_out = ram[mem_sel];
    end
  end
  ...
  always_comb begin
    ...
    //   rgb2[1] = pwm_signal;
    // end else begin
      rgb1[1] = (&ctrl_syscall) ? pwm_signal : 1'b0; 
      rgb2[1] = (&ctrl_syscall) ? pwm_signal : 1'b0;
      rgb1[2] = (out_empty) ? 1'b0 : pwm_signal;
      rgb2[2] = (out_empty) ? 1'b0 : pwm_signal;
      rgb1[0] = (rd_uart) ? 1'b0 : pwm_signal1; // Needs to be brighter to be seen
      rgb2[0] = (pgm_mem) ? 1'b0 : pwm_signal1; // Needs to be brighter to be seen
    // end
  end

  // Scrolling up and down the memory 
  logic is_btnU, is_btnD, is_btnR, is_btnL, pgm_mem_pedge;
  ...
  button u_btn_pgm_mem(.clk(clk), .reset_n(1'b1), .noisy(~pgm_mem), .debounced(), .p_edge(pgm_mem_pedge), .n_edge(), ._edge());
  
  udl_counter #(.BITS(12)) DEBOUNCED_BUTTON_COUNTER (
      .clk(clk),
      .reset_n(1'b1),
      .enable(is_btnD || is_btnU || is_load || pgm_mem_pedge),
  ...
  FIFO_ByteToWord_Controller u_fifo_bytetoword_controller(
      .clk(clk),
      .rst(~is_CPU_RESETN),
      .rd_btn(rd_uart_pedge || pgm_mem_pedge),
  ...
  always_comb begin
      case (digit_sel)
          3'b000: begin
              ...
              an = 8'b1110_1111;
              sseg = (btnR || is_load || ~|sw || out_empty) ? {1'b0, seg_pattern} : {1'b1, seg_pattern};
          end
          3'b001: begin
              if (~out_empty && ~btnR)
                curr = out_data[7:4];
  ...
  assign LED = (out_empty) ? sw : out_data;
